...
def chunk_text(text, chunk_size=128, overlap=20):
    words = text.split()
    chunks = []
...
for i, rows in enumerate(tqdm(dataset, desc="Processing documents", total=NB_ROWS)):
    if i >= NB_ROWS:
        break
...
faiss.write_index(index, os.path.join(ROOT_DIR, "my_rag_db.index"))
...
with open(os.path.join(ROOT_DIR, "my_rag_db.json"), "w") as f:
